...
function GroupInfo() {
  ...
  const generatePDF = async () => {
    ...
    const text = `${group?.teacher.user.user_lastname} ${group?.teacher.user.user_name}`;
const fontSize = 8;
...
const textStartX = centerX - (textWidth / 2);
    ...
    doc.text(`${group?.teacher.user.user_lastname} ${group?.teacher.user.user_name}`, textStartX, tableY + 37);
    doc.text(`${group?.teacher.user.user_ci}`, 96.5, tableY + 43);
        ...
        <div className=" block m-3">
          ...
          <div className=" flex justify-center items-center">
            <div className=" my-2 grid grid-cols-2">
              <div className=" mx-4">
                <button onClick={()=> removeGroup(groupId2)} className=" p-2 active:transform active:scale-90 border border-white bg-[#ad2c2c] rounded-lg hover:bg-[#b94d4d] text-white hover:text-white text-[12px] md:text-sm lg:text-base duration-500">
                  Eliminar
                </button>
              </div>
              <div className=" mx-4">
